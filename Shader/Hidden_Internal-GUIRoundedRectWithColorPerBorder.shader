...
vec4 _BorderColorsDynamicIndex(int i){
#if UNITY_DYNAMIC_INDEX_ES2
    return _BorderColors[i];
#else
#define d_ar _BorderColors
    if (i <= 0) return d_ar[0]; else if (i == 1) return d_ar[1]; else if (i == 2) return d_ar[2]; else if (i == 3) return d_ar[3];
    return d_ar[0];
#undef d_ar
#endif
...
void main()
{
    ...
    u_xlat9.xyz = max(u_xlat16_9.xyz, vec3(0.0, 0.0, 0.0));
    u_xlat16_2.xyz = (_ManualTex2SRGB != 0) ? u_xlat9.xyz : u_xlat10_1.xyz;
    ...
    u_xlat3.y = _Rect[0] + _CornerRadiusesDynamicIndex(u_xlati27);
    u_xlat4.y = _Rect[1] + _CornerRadiusesDynamicIndex(u_xlati27);
    u_xlat3.w = _BorderWidths[0] + _Rect[0];
    u_xlat4.w = _BorderWidths[1] + _Rect[1];
    u_xlat5.x = _Rect[0] + _Rect[2];
    u_xlat5.y = u_xlat5.x + (-_CornerRadiusesDynamicIndex(u_xlati27));
    ...
    u_xlat6.y = u_xlat6.x + (-_CornerRadiusesDynamicIndex(u_xlati27));
    ...
    u_xlatb21 = 0.0<_CornerRadiusesDynamicIndex(u_xlati27);
    ...
    if(u_xlati18 == 0) {
        ...
        u_xlat16_29 = u_xlat10_1.w * vs_COLOR0.w;
    } else {
        ...
        u_xlat8.xyz = u_xlat16_2.xyz * _BorderColorsDynamicIndex(u_xlati18).xyz;
        u_xlat29 = u_xlat10_1.w * _BorderColorsDynamicIndex(u_xlati18).w;
    ...
    u_xlat1 = u_xlat1 + (-_CornerRadiusesDynamicIndex(u_xlati27));
    u_xlat1 = u_xlat1 * u_xlat0.x + 0.5;
    u_xlat1 = clamp(u_xlat1, 0.0, 1.0);
    u_xlat1 = u_xlatb18 ? u_xlat1 : float(0.0);
    u_xlat19 = (-u_xlat5.z) + _CornerRadiusesDynamicIndex(u_xlati27);
    u_xlat27 = (-u_xlat6.z) + _CornerRadiusesDynamicIndex(u_xlati27);
    ...
    u_xlat9.z = texture2D(_GUIClipTexture, vs_TEXCOORD1.xy).w;
